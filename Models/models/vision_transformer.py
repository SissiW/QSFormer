...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        # ================== diffusion ==================
        attn_alpha = self.alpha * attn
        attn_item = attn_alpha
        sigma = attn_alpha
        for t in range(self.T):
            if t == 0:
                continue
            else:
                attn_item = torch.einsum('ijkm,ijml->ijkl', [attn_item, attn_alpha.transpose(-2, -1)])
                sigma = sigma + attn_item
            
        attn = attn_item + (1-self.alpha) * sigma 
        # ================================================'''
...
class Block(nn.Module):
    ...
    def forward(self, x):  # torch.Size([495, 197, 384])
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))  # torch.Size([495, 197, 384])
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x))))  # torch.Size([495, 197, 384])
        return x
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        # print(self.representation_size, final_chs, num_classes)  # None 384 1000
    ...
    def forward_features(self, x):
        # x = self.patch_embed(x)  # torch.Size([495, 196, 384])
        # x = torch.cat((self.cls_token.expand(x.shape[0], -1, -1), x), dim=1)  # torch.Size([495, 197, 384])
        x = self.pos_drop(x + self.pos_embed)  # torch.Size([495, 197, 384])
        # if self.grad_checkpointing and not torch.jit.is_scripting():
        #     x = checkpoint_seq(self.blocks, x)
        # else:
        x = self.blocks(x)  # torch.Size([495, 197, 384])
        x = self.norm(x)
        return x

    def forward_head(self, x, pre_logits: bool = False):  # torch.Size([495, 197, 384]) False
        if self.global_pool:
            x = x[:, 1:].mean(dim=1) if self.global_pool == 'avg' else x[:, 0]  # torch.Size([495, 384])
        x = self.fc_norm(x)  # nn.Identity() torch.Size([495, 384]) 
        x = self.pre_logits(x)  # torch.Size([495, 384])
        return x if pre_logits else self.head(x)  # nn.Identity()

    def forward(self, x):  # torch.Size([495, 3, 224, 224])
        x = self.forward_features(x)  #  torch.Size([495, 197, 384])
        # x = self.forward_head(x)  # torch.Size([495, 384])
        return x
